    ...
    public class worldStrandPathEditor : UnityEditor.Editor
    {
        ...
        void DrawProfileCurve(SerializedProperty list)
        {
            ...
            if (Event.current.type == EventType.MouseDown && Event.current.clickCount == 2 && rect.Contains(Event.current.mousePosition))
            {
                ...
                {
                    SerializedProperty p0 = list.GetArrayElementAtIndex(i);
                    SerializedProperty p1 = list.GetArrayElementAtIndex(i + 1);
                    float x0 = Mathf.Clamp01(p0.FindPropertyRelative("x").floatValue);
                    float y0 = Mathf.Clamp01(p0.FindPropertyRelative("y").floatValue);
                    float x1 = Mathf.Clamp01(p1.FindPropertyRelative("x").floatValue);
                    float y1 = Mathf.Clamp01(p1.FindPropertyRelative("y").floatValue);
                    Vector2 a = new Vector2(x0, y0);
                    Vector2 b = new Vector2(x1, y1);
                    Vector2 p = new Vector2(x, y);
                    float dist = HandleUtility.DistancePointLine(p, a, b);
                    if (dist < minDist)
                    {
                        minDist = dist;
                        insertIndex = i + 1;
                ...
                {
                    list.arraySize++;
                    insertIndex = list.arraySize - 1;
                }
                SerializedProperty np = list.GetArrayElementAtIndex(insertIndex);
